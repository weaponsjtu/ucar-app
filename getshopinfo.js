	...
	$.getJSON('http://ihavecar.sinaapp.com/shopinfo.php?shop_id='+shop+'&type='+type+'&jasoncallback=?&randomID='+Math.random(), function(json){
		...
		$.each(json,function(entryIndex,entry){
	      ...
	      html+='<div style="font-size: 18px;"><b>'+entry['业户名称']+'</b></div>'; 
				...
				html+='<div><a href="tel:62935474">许可证号：'+entry['许可证号']+'</a></div>'; 
				html+='<hr size="1" color="#AAA"   align="center" />';
	            html+='<div>地址：'+entry['经营地址']+'</div>';   
				tag += entry['经营范围'];
			...
			if (type == '2') {
				html+='<div style="font-size: 18px;"><b>'+entry['加油站名称']+'</b></div>'; 
				html+='<hr size="1" color="#AAA"   align="center" />';
				html+='<div><a href="tel:' + entry['联系电话'] +'">电话：'+entry['联系电话']+'</a></div>'; 
				html+='<hr size="1" color="#AAA"   align="center" />';
	            html+='<div>地址：'+entry['加油站地址']+'</div>';   
				tag += entry['加油站属性'];
				star += '<span id="star"></span><font style="float:right">共有' + entry['评分人数'] + '个评分</font><script type="text/javascript">$(function(){$("#star").raty({readOnly:	true,start: '+entry['评分总和']+'	});});</script>'; 
			}
			$('#lng').html(entry['经度']);
			$('#lat').html(entry['纬度']);
	  });  
