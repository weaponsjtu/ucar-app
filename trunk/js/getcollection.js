	...
	$(document).ready(function(){
		...
		//random函数用来取消缓存影响,jsonp形式进行跨域传数据
			$.each(json,function(entryIndex,entry){
				html+='<tr>';
				html+='<td><div id="shopIdType'+entry['shopid']+'" style="display:none">'+entry['userid']+'#'+entry['shopid']+'#'+entry['shoptype']+'</div></td>';
				...
				html+='<td id="wrap_delete"> <a href="#" class="delete" onclick="shopDelete(event,'+entry['shopid']+')">Delete</a> </td>';
	            html+='</tr>';  
	      });
			html+='</table>';   
